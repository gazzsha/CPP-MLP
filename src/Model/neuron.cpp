...
GraphPreceptron::GraphPreceptron(const size_t& size_enter, const size_t& n,
                                 const size_t& sz, const size_t& output_size)
...
void GraphPreceptron::DirectPassage() {
  ...
  for (size_t i = 0; i < shadow_layers[0].get_neurons().size(); ++i) {
    double sum = 0;
    for (size_t j = 0; j < shadow_layers[0].get_neurons()[i].get_prev().size();
         ++j) {
      sum += (shadow_layers[0].get_neurons()[i].get_prev()[j])->get_value() *
  ...
  for (size_t i = 1; i != size; ++i) {
    for (size_t j = 0; j < shadow_layers[i].get_neurons().size(); ++j) {
      double sum = 0;
      for (size_t k = 0;
           k < shadow_layers[i].get_neurons()[j].get_weights().size(); ++k) {
        sum += (shadow_layers[i].get_neurons()[j].get_weights()[k] *
                shadow_layers[i - 1].get_neurons()[k].get_value());
      }
      shadow_layers[i].get_neurons()[j].set_value(get_function_value(sum));
  ...
  for (size_t i = 0; i != output_layer.get_neurons().size(); ++i) {
    double sum = 0;
    for (size_t j = 0; j != output_layer.get_neurons()[i].get_weights().size();
         ++j) {
      sum += output_layer.get_neurons()[i].get_weights()[j] *
             shadow_layers[size - 1].get_neurons()[j].get_value();
    }
...
void GraphPreceptron::Predict(const std::vector<double>& vec) {
  InputEnterLayer(vec);
