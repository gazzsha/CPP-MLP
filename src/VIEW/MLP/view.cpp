...
View::View(s21::Controller* controller_, QWidget* parent)
    : controller(controller_), QMainWindow(parent), ui(new Ui::View) {
  ui->setupUi(this);
...
void View::on_pushClear_clicked() { scene->clear(); }
...
void View::InitWidget() {
  ...
  ui->widget->plotLayout()->addElement(
      0, 0,
      new QCPTextElement(ui->widget, "Metrics report",
                         QFont("Arial", 26, QFont::Bold)));
}
...
void View::on_push_training_data_clicked() {
  ...
  file_path = QFileDialog::getOpenFileName(
      this, " Select File", "../../../../../", "All Files(*.*);; CSV(*.csv)");
...
void View::on_push_testing_data_clicked() {
  QString file_path;
  file_path = QFileDialog::getOpenFileName(
      this, " Select File", "../../../../../", "All Files(*.*);; CVS(*.csv)");
...
void View::on_push_check_testing_data_clicked() {
  ...
  ui->accuracy_label->setText(
      QString::number(controller->get_average_accuracy()));
...
void View::on_push_load_weights_clicked() {
  QString file_path;
  file_path = QFileDialog::getOpenFileName(this, "Select File",
                                           "../../../../../Weights/",
                                           "All Files(*);; TXT(*.txt)");
  try {
...
void View::on_push_save_weights_clicked() {
  QString file_path;
  file_path = QFileDialog::getSaveFileName(this, "Select File",
                                           "../../../../../Weights/",
                                           "All Files(*);; TXT(*.txt)");
  try {
...
void View::on_push_load_bmp_clicked() {
  ...
  if (img.height() > 512 || img.width() > 512 || file_path.isEmpty()) {
    ...
    QImage new_image = grayscale_image.scaled(28, 28, Qt::IgnoreAspectRatio,
...
void View::on_push_train_online_clicked() {
  ...
  for (size_t i = 0; i < controller->get_epochs(); ++i) {
    ...
    ui->tableWidget->setItem(i, 1,
                             new QTableWidgetItem(QString::number(
                                 controller->get_average_accuracy_vec()[i])));
    ui->tableWidget->setItem(i, 2,
                             new QTableWidgetItem(QString::number(
                                 controller->get_precision_vec()[i])));
    ui->tableWidget->setItem(
        i, 3,
        new QTableWidgetItem(QString::number(controller->get_recall_vec()[i])));
    ui->tableWidget->setItem(i, 4,
                             new QTableWidgetItem(QString::number(
                                 controller->get_f_measure_vec()[i])));
    ui->tableWidget->setItem(
        i, 5,
        new QTableWidgetItem(QString::number(controller->get_time_vec()[i])));
  }
  ...
  QVector<double> x_data =
      QVector<double>(x_data_std.begin(), x_data_std.end());
  QVector<double> average_accuracy =
      QVector<double>(average_accuracy_std.begin(), average_accuracy_std.end());
  QVector<double> precision =
      QVector<double>(precision_std.begin(), precision_std.end());
  QVector<double> recall =
      QVector<double>(recall_std.begin(), recall_std.end());
  QVector<double> f_measure =
      QVector<double>(f_measure_std.begin(), f_measure_std.end());
  QVector<double> time = QVector<double>(time_std.begin(), time_std.end());

  ui->widget->setLocale(
      QLocale(QLocale::English,
              QLocale::UnitedKingdom));  // period as decimal separator and
  ...
  red_pen.setWidthF(3);
  ui->widget->graph(1)->setScatterStyle(
      QCPScatterStyle(QCPScatterStyle::ssDisc, 7));
  ...
  ui->widget->graph(2)->setScatterStyle(
      QCPScatterStyle(QCPScatterStyle::ssDisc, 7));
  ...
  ui->widget->graph(3)->setScatterStyle(
      QCPScatterStyle(QCPScatterStyle::ssDisc, 7));
  ...
  ui->widget->graph(4)->setScatterStyle(
      QCPScatterStyle(QCPScatterStyle::ssDisc, 7));
  ...
  ui->widget->yAxis2->setRange(
      0, *(std::max_element(time.begin(), time.end())) +
             0.1 * *(std::max_element(time.begin(), time.end())));
  ...
  ui->widget->replot();
  ui->accuracy_label->setText(
      QString::number(controller->get_average_accuracy_of_full_train()));
  ui->precision_label->setText(
      QString::number(controller->get_average_precision_of_full_train()));
  ui->recall_label->setText(
      QString::number(controller->get_average_recall_of_full_train()));
  ui->f_mera_label->setText(
      QString::number(controller->get_average_f_measure_of_full_train()));
  ui->time_label->setText(
      QString::number(controller->get_full_time_of_full_train()));
}
...
void View::on_MatrixRadio_clicked() { controller->SwitchModel(); }

void View::on_GraphRadio_clicked() { controller->SwitchModel(); }
